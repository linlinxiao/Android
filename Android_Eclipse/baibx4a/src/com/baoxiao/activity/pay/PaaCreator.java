...
public class PaaCreator {
	public static JSONObject randomPaa() {
	    ...
	    try {   //按照要求传入所有数据即可，下面的那个数组也要对应写好，因为它有方法自动进行MD5加密生成密钥key
			paaParams.put("inputCharset", "1");
			paaParams.put("receiveUrl", "https://service.allinpay.com/gateway/index.do");
			paaParams.put("version", "v1.0");
			paaParams.put("signType", "0");           //2015.11.2 改动了这里!
			paaParams.put("merchantId", "109020201510019"); 
			paaParams.put("orderNo", "NOM"+timeStr);	//my:订单号加入了系统时间来识别
			paaParams.put("orderAmount",ProductPay.orderAmount);  //my:直接使用类名调用静态变量
			paaParams.put("orderCurrency", "0");
			paaParams.put("orderDatetime", timeStr);			
			paaParams.put("productName", "用户"+ProductPay.userid+"：百宝箱业务开通!"); //加入用户id识别
//			paaParams.put("ext1", ext1FromInput());				
//			paaParams.put("ext1", "<USER>id</USER>");
			paaParams.put("payType", "0"); 			 //2015.11.2 改动了这里!
...
//			paaParams.put("language", "3");
		    ...
		    String[] paaParamsArray = {
				"1","inputCharset",
				"https://service.allinpay.com/gateway/index.do","receiveUrl",
				"v1.0","version",
//				"3","language",
				"0","signType",    //2015.11.2 改动了这里!
				"109020201510019","merchantId",
				"NOM"+timeStr,"orderNo",
				ProductPay.orderAmount,"orderAmount",
				"0","orderCurrency",
				timeStr,"orderDatetime",
				"用户"+ProductPay.userid+"：百宝箱业务开通!", "productName",
//				ext1FromInput(),"ext1",
//			    "<USER>201406231006545</USER>","ext1",
				"0","payType", //2015.11.2 改动了这里!
//				"visa","issuerId",
//				"GOODS","tradeNature",
				"1234567890","key",
		    };
		    ...
		    try {
				paaParams.put("signMsg", md5Str);       //这里会生成md5签名
