...
public class PingAnFuService {

	public ArrayList<HongLi> findHongLi(HongLi hongli, Context c, String type) {

		ArrayList<HongLi> list = DBDAO.findHongli(hongli, c, type,Define.tableName_hongLi);
...
//		}
	...
	//附加意外医疗保费的计算
	public double findYiLiaoBaoFei(String baoFei) {
		Double yiWai = Arithmetic4Double.add(
				Arithmetic4Double.div(
				Arithmetic4Double.multi(
				Arithmetic4Double.sub(
				Arithmetic4Double.multi(
						Double.valueOf(baoFei),10000)
						,1000)
						,6)
						,1000)
						,24);
	...
	public double findhuoMianBaoE(HongLi hongli,Context c,String type,String baoE
			 	,String zhongJBaoE,Double yiWaiBaoFeiD,SQLiteDatabase db) {
		double baoFei = 0;
		//主保费费率
		baoFei = Double.valueOf(DBDAO.findBaoFei(hongli, c, type,db));
		baoFei = Arithmetic4Double.add(
				Arithmetic4Double.multi(
				Arithmetic4Double.sub(Integer.valueOf(baoE), Integer.valueOf(zhongJBaoE)), baoFei),
				yiWaiBaoFeiD);
		return baoFei;
	}
